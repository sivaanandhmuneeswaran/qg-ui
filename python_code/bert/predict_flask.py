...
def read_squad_examples(input_data, is_training, version_2_with_negative):
    ...
    examples = []
    tagged_paragraph_text = input_data["tagged_sentence"]
    paragraph_text = input_data["original_sentence"]
    doc_tokens = []
    char_to_word_offset = []
    ...
    i = 1
    for j in range(0,len(input_data["question"])):
        qas_id = str(input_data["index"]) + str(i)
        i = i+1
        answer = input_data["answer"]
        question_text = input_data["question"][j]
        score = input_data["score"][j]
        start_position = None
        end_position = None
        orig_answer_text = None
        is_impossible = False
...
def write_predictions(all_examples, all_features, all_results, n_best_size,
    ...
    for (example_index, example) in enumerate(all_examples):
        ...
        if(is_impossible):
            ...
            finalresult.append({"index":example.qas_id,"original_sentence":(" ".join(example.doc_tokens)),"tagged_sentence":example.tagged_sentence,"answer":example.ans,"score":example.score,"question":example.question_text,"is_impossible":is_impossible})
    ...
    for i in range(0,len(input)):
        ...
        if(index != int(input[i+1]["index"])//10 ):
            flag = 0
            res.append({'index':fin_index,'original_sentence':input[i]["original_sentence"],'tagged_sentence':input[i]["tagged_sentence"],'answer':input[i]["answer"],'question':question,'score':score})
